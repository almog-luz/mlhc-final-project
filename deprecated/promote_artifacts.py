...
def _resolve_path(preferred: str, legacy: str) -> Path:
    p = Path(preferred)
    if p.exists():
        return p
    ...
    if legacy_path.exists():
        return legacy_path
    # Return preferred even if missing; later code will raise clearer errors
    return p

PROMOTED_DIR = _resolve_path('artifacts', 'project/artifacts')
RUNS_ROOT = _resolve_path('runs', 'project/runs')
